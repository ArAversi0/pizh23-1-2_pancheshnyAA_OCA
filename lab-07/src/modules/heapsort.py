...
def heapsort(array):
    ...
    while heap.peek() is not None:  # N итераций
        extracted_value = heap.extract()  # O(log N)
        sorted_array.append(extracted_value)  # O(1)
...
def heapsort_inplace(array):
    ...
    def sift_down_max(n_size, idx):
        ...
        if largest != idx:
            array[idx], array[largest] = array[largest], array[idx]  # O(1) swap
