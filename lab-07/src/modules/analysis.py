...
def measure_sorting():
    ...
    for size in sizes:
        data = [random.randint(0, 100000) for _ in range(size)]
        data_copy = data[:]  # лёгкое улучшение читаемости

        # 1. Heapsort In-place (O(N log N))
        t_heap = (timeit.timeit(lambda: heapsort_inplace(data[:]), number=5) / 5) * 1000
        times_heapsort.append(t_heap)

        # 2. Python Sorted (Timsort)
        t_py = (timeit.timeit(lambda: sorted(data_copy), number=5) / 5) * 1000
